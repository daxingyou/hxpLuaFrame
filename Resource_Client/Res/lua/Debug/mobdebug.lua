...
local function printBreakpoints()
	for k,v in pairs(breakpoints) do
		for k1,v1 in pairs(v) do
			print("line=" .. tostring(k) .. " file=" .. tostring(k1) .. " v=" .. tostring(v1))
		end
	end
end

local function set_breakpoint(file, line)
  print("set_breakpoint,file=" .. file .. " line=".. tostring(line))
...
local function remove_breakpoint(file, line)
  print("remove_breakpoint,file=" .. file .. " line=".. tostring(line))
  if file == '-' and lastfile then file = lastfile
  elseif iscasepreserving then file = string.lower(file) end
...
local function has_breakpoint(file, line)
  print("has_breakpoint,file=" .. file .. " line=".. tostring(line))
...
local function debug_hook(event, line)
  ...
  elseif event == "line" then
	...
	local csource = caller.source
	local pos1,pos2=csource:lower():find("scripts")
	if pos2 then
		csource =  csource:sub(pos2+2)
	end
	
	print("debug_hook,lastfile=" .. tostring(lastfile) .. " lastsource=" .. tostring(lastsource) .. " csource=" .. tostring(csource) .. " caller.source=" .. tostring(caller.source))

    -- grab the filename and fix it if needed
    local file = lastfile
    if (lastsource ~= csource) then
      file, lastsource = csource, csource
    ...
    if (watchescnt > 0) then
      ...
      for index, value in pairs(watches) do
        ...
        if ok and fired then
		  print("debug_hook:coroutine.resume,events.WATCH")
    ...
    if getin then
      ...
      step_into = false
      step_over = false
	  print("debug_hook:coroutine.resume,events.BREAK")
    ...
    if status and res == 'stack' then
      while status and res == 'stack' do
		...
		print("debug_hook:coroutine.resume,events.STACK or events.BREAK")
...
local function debugger_loop(sev, svars, sfile, sline)
  ...
  while true do
    ...
    while true do
	  ...
	  print("debugger_loop: line=" .. tostring(line))
    ...
    elseif command == "RUN" then
	  ...
	  print("debugger_loop: RUN file=" .. tostring(file) .. " line=" .. tostring(line))
      eval_env = vars
    ...
    elseif command == "STEP" then
      ...
      step_into = true
      local ev, vars, file, line, idx_watch = coroutine.yield()
	  print("debugger_loop: STEP file=" .. tostring(file) .. " line=" .. tostring(line))
      eval_env = vars
      if ev == events.BREAK then
        server:send("202 Paused " .. file .. " " .. line .. "\n")
      elseif ev == events.WATCH then
    ...
    elseif command == "OVER" or command == "OUT" then
      ...
      local ev, vars, file, line, idx_watch = coroutine.yield()
	  print("debugger_loop: OVER&OUT file=" .. tostring(file) .. " line=" .. tostring(line))
      eval_env = vars
      if ev == events.BREAK then
        server:send("202 Paused " .. file .. " " .. line .. "\n")
      elseif ev == events.WATCH then
    ...
    elseif command == "STACK" then
      ...
      local vars, ev = {}
      if seen_hook then
		print("debugger_loop:coroutine.yield stack")
    ...
    while true do
      ...
      if abort then
        ...
        elseif err and not tostring(err):find(deferror) then
		  ...
		  print("controller:coroutine.resume")
