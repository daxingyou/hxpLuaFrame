...
local RAFU_Cfg_OribitCalc_MultiMissile = {
	--抛物线用到的配置
	ParabolaDirCfg = {		
		-- 根据实际角度控制，应该具有以下情况
		-- 1、15参数一致，5、11参数一致  	
		...
		[FU_DIRECTION_ENUM.DIR_UP             ] = {			
			Speed_Y_Init_V_Main = 100,		-- 自身垂直方向上的初始速度
			Distance_Y_Percent = 0.01,		-- 炮弹向下走的路程和直线距离的百分比
		...
		[FU_DIRECTION_ENUM.DIR_DOWN           ] = {
			Speed_Y_Init_V_Main = 100,		-- 自身垂直方向上的初始速度
			Distance_Y_Percent = 0.01,		-- 炮弹向上走的路程和直线距离的百分比
			AheadTimeToRemoveBody = 0.1,
		}, 
...
return RAFU_Cfg_OribitCalc_MultiMissile