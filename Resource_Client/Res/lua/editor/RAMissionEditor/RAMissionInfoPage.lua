...
local RAMissionInfoPage = BaseFunctionPage:new(...)
...
function RAMissionInfoPage:Enter(data)
...
function RAMissionInfoPage:initData()
...
function RAMissionInfoPage:_initTitle()
    local titleCCB = UIExtend.getCCBFileFromCCB(self.ccbfile, "mCommonTitleCCB")
	local RACommonTitleHelper = RARequire('RACommonTitleHelper')
	local backCallBack = function()
		RARootManager.CloseCurrPage()
	end
    local titleName = "选择关卡"
	local titleHandler = RACommonTitleHelper:RegisterCommonTitle('RAMissionInfoPage', 
    titleCCB, titleName, backCallBack, RACommonTitleHelper.BgType.Blue)
end

function RAMissionInfoPage:createTTF(pos,text)
...
function RAMissionInfoPage:initMissionPanel()
...
function RAMissionInfoPage:CommonRefresh()
...
function RAMissionInfoPage:onCommit()
    -- RAUnload('TestData')
    -- local TestData = RARequire('TestData')
    -- TestData.missionId = getValue(self.missionEditBox)
    -- if TestData.missionId == 0 then 
    ...
    local missionId = getValue(self.missionEditBox)
    if missionId == 0 then 
        RARootManager.ShowMsgBox(_RALang("没有填关卡ID"))
        return 
    end

    local battle_mission_conf = RARequire('battle_mission_conf')
    local curMissionData = battle_mission_conf[missionId]

    if curMissionData == nil then 
        RARootManager.ShowMsgBox(_RALang("没有这个关卡ID"))
        return 
    end 
    RARequire('RAMissionScene'):changeMission(curMissionData)
    RARootManager.ClosePage('RAMissionInfoPage')
end

function RAMissionInfoPage:Exit()
    mAllScrollview:removeAllCell()
    local RACommonTitleHelper = RARequire('RACommonTitleHelper')
	RACommonTitleHelper:RemoveCommonTitle("RAMissionInfoPage")
    self.container:removeFromParentAndCleanup(true)
    UIExtend.unLoadCCBFile(self)
end

return RAMissionInfoPage