...
local ELEC_GRID_MAX = 18        -- for 0107  电力最大各自块数是18
...
RAMainUIBottomBanner.lastData = {
    ...
    mGreenGridNum = 0,      --电力绿色格子
    mYellowGridNum = 0,     --电力黄色格子
    mRedGridNum = 0,        --电力红色格子
}
...
function RAMainUIBottomBanner:Enter(data)
	...
	local ccbfile = UIExtend.loadCCBFile("ccbi/RAMainUIBottomBannerNew.ccbi",RAMainUIBottomBanner)
    ...
    self.mBattleNode = UIExtend.getCCNodeFromCCB(ccbfile, "mBattleNode")
    self.mBattleNode:setVisible(true)
    ...
    if self.mCityGainNode and self.mActivityNode and self.mNuclearCDNode and self.mLittleHelperNode then
        if RAGuideManager.isInGuide() then
            ...
            UIExtend.setNodeVisible(ccbfile, "mBubbleNode", false)
...
onReceiveMessage = function(msg)
    ...
    if msg.messageID == MessageDef_MainUI.MSG_ShowHelperTips then
        ...
        RAMainUIBottomBanner:setLittleHelperNodeShow( true )
    ...
    if msg.messageID == MessageDef_MainUI.MSG_HideHelperTips then
        ...
        local RAGuideManager = RARequire("RAGuideManager")
        if RAGuideManager.isInGuide() then
            RAMainUIBottomBanner:setLittleHelperNodeShow( false )
        end
        return
    end      
    ...
    if msg.messageID == MessageDef_Guide.MSG_Guide then
        ...
        elseif keyWord == RAGuideConfig.KeyWordArray.ShowElectricLow then
            ...
            contenSize.width = contenSize.width + RAGuideConfig.GuideTips.ConfigOffset*2
            contenSize.height = contenSize.height + RAGuideConfig.GuideTips.ConfigOffset*2
        ...
        elseif keyWord == RAGuideConfig.KeyWordArray.CircleTaskBtn then
            local taskNode = UIExtend.getCCNodeFromCCB(RAMainUIBottomBanner.ccbfile, "mGuildTaskNode")
            if taskNode then
...
function RAMainUIBottomBanner:refreshTask()
    ...
    if taskInfo then
        ...
        if taskInfo.taskState == RAGameConfig.TaskStatus.Complete and self.taskCCB then
            self.taskCCB:runAnimation("ToCanReceiveAni")
        else 
            self.taskCCB:runAnimation("UnFinishLoopAni")
        end
...
function RAMainUIBottomBanner:updateElectric(isNoAni)
    ...
    --begain: for 0107 修改
    local checkGridValid = function(gridNum)
        if gridNum > ELEC_GRID_MAX then
            return ELEC_GRID_MAX
        elseif gridNum <= 0 then
            return 1
        end
        return gridNum
    end

    local scale2GridNum = function(scale)
        if scale < 0 then
            return 0
        elseif scale > 1 then
            return ELEC_GRID_MAX
        ...
        return scale * ELEC_GRID_MAX
    end


    local greenGridNum = 0
    local yellowGridNum = 0
    local redGridNum = 0

    greenGridNum = math.ceil(scale2GridNum(greenScaleTo))
    greenGridNum = checkGridValid(greenGridNum)
    yellowGridNum = math.floor(scale2GridNum(yellowScaleTo))
    yellowGridNum = checkGridValid(yellowGridNum)
    redGridNum = math.floor(scale2GridNum(redScaleTo))
    redGridNum = checkGridValid(redGridNum)

    if isNoAni then
        --显示红色格子
        if self.lastData.mRedGridNum ~= redGridNum then
            for k=1, redGridNum do
                UIExtend.setNodeVisible(self:getRootNode(), "mPowerBar"..k, true)
                UIExtend.setSpriteIcoToNode(self:getRootNode(), "mPowerBar"..k, "NewMainUI_u_PEBar_Red.png")
            end
        end

        --显示黄色格子
        if self.lastData.mYellowGridNum ~= yellowGridNum then
            for k=redGridNum+1, yellowGridNum do
                UIExtend.setNodeVisible(self:getRootNode(), "mPowerBar"..k, true)
                UIExtend.setSpriteIcoToNode(self:getRootNode(), "mPowerBar"..k, "NewMainUI_u_PEBar_Yellow.png")
            end
        end
        --显示绿色格子
        if self.lastData.mGreenGridNum ~= greenGridNum then
            local maxYellowOrRed = math.max(redGridNum, yellowGridNum)
            for k=maxYellowOrRed+1, greenGridNum do
                UIExtend.setNodeVisible(self:getRootNode(), "mPowerBar"..k, true)
                UIExtend.setSpriteIcoToNode(self:getRootNode(), "mPowerBar"..k, "NewMainUI_u_PEBar_Green.png")
            end
        end

         --隐藏剩余格子
        local maxYellowOrRedOrGreen = math.max(maxYellowOrRed, greenGridNum)
        for k=greenGridNum+1, ELEC_GRID_MAX do
            UIExtend.setNodeVisible(self:getRootNode(), "mPowerBar"..k, false)
        end
    else
         --红色格子
         if self.lastData.mRedGridNum ~= redGridNum then
            RAActionManager:CreateGridProcessBarAction(self:getRootNode(), "mPowerBar", 1, redGridNum, "NewMainUI_u_PEBar_Red.png")
         end
        --黄色格子
        if yellowGridNum > redGridNum and self.lastData.mYellowGridNum ~= yellowGridNum then
            RAActionManager:CreateGridProcessBarAction(self:getRootNode(), "mPowerBar", redGridNum+1, yellowGridNum, "NewMainUI_u_PEBar_Yellow.png")
        end
        --绿色格子
        local maxYellowOrRed = math.max(redGridNum, yellowGridNum)
        if greenGridNum > maxYellowOrRed and self.lastData.mGreenGridNum ~= greenGridNum then
            RAActionManager:CreateGridProcessBarAction(self:getRootNode(), "mPowerBar", maxYellowOrRed+1, greenGridNum, "NewMainUI_u_PEBar_Green.png")
        end
    ...
    self.lastData.mRedGridNum = redGridNum
    self.lastData.mYellowGridNum = yellowGridNum
    self.lastData.mGreenGridNum = greenGridNum
    --end: for 0107 修改
...
function RAMainUIBottomBanner:onWorldBtn()
    ...
    --如果下一步是去剧情scene，这里直接gotoNext
    local keyworld = RAGuideManager.getKeyWordById()
    if keyworld == RAGuideConfig.KeyWordArray.StartStory then
        RAGuideManager.gotoNextStep()
        return
    end
...
--    RAMissionVar:setFragmentId(10000002)
--    RAMissionBarrierManager:gotoBarrier(20000001)
    
end

function RAMainUIBottomBanner:onBattleBtn()
...
function RAMainUIBottomBanner:mTargetTipsNode_onTargetTipsBtn()
    local taskInfo = RATaskManager.getRecommandTask()
    if self.clickTargetTipsBtn then return end
    self.clickTargetTipsBtn=true
...
function RAMainUIBottomBanner:refreshChatMsg()
    ...
    --for i = 1,#self.mainUIChatMsgs do
        ...
        local data = self.mainUIChatMsgs[1]
        ...
        mChatTex1:setString(content)
        RAStringUtil:resetHTMLStringWidth(mChatTex1, 500)--465)    
    --end
end
...
function RAMainUIBottomBanner:showWorldDirection(distance, degree, hideArrow)
    ...
    local str = distance .. '\n' .. RAStringUtil:getLanguageString('@KM')
